...
///
/// Worth showing plainly, because AI Notch never signs in — it borrows a
...
///
/// AI Notch cannot sign anyone in — it reads a credential the owning tool
...
enum SignInRoute: Equatable {
    ...
    /// The only case where AI Notch genuinely signs anyone in or out.
    ...
    var signOutCaveat: String {
        switch self {
        case .modal(let name):
            return "Signs out of \(name) — the session belongs to AI Notch."
