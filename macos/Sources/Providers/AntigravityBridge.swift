...
/// because the API judges *which client* is asking and AI Notch cannot
...
enum AntigravityBridge {
    ...
    private static func quota(port: Int, token: String,
        ...
        let (data, response) = try await ProviderHTTP.data(for: request, using: session, hosts: ["127.0.0.1"])
