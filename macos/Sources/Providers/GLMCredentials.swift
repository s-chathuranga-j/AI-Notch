...
enum GLMCredentials {
    ...
    static func claudeCode(_ url: URL) -> Credential? {
        ...
        guard let base = string(env["ANTHROPIC_BASE_URL"]),
              let url = URL(string: base), url.scheme == "https",
    ...
    static func zcodePlanKey(_ url: URL) -> Credential? {
        ...
        for (id, value) in providers.sorted(by: { $0.key < $1.key }) {
            guard ["builtin:zai-coding-plan", "builtin:bigmodel-coding-plan"].contains(id), let provider = value as? [String: Any],
            ...
            guard let base = string(options["baseURL"]),
                  let url = URL(string: base), url.scheme == "https",
                  url.user == nil, url.password == nil,
                  let host = url.host, isZaiHost(host),
