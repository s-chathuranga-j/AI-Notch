...
actor GLMProvider: UsageProvider {
    ...
    init(session: URLSession = ProviderHTTP.session(), archive: UsageArchive = UsageArchive()) {
    ...
    private func fetch(credentials: GLMCredentials.Credential) async throws -> Data {
        ...
        let (data, response) = try await ProviderHTTP.data(for: request, using: session, hosts: ["api.z.ai", "open.bigmodel.cn"])
