...
enum CodexBridge {
    ...
    /// Cancellation follows the provider switch into the child process.
    ...
    static func cancellableRateLimits(executable: URL) async throws -> Data {
        let cancellation = ProcessCancellation()
        return try await withTaskCancellationHandler {
            try Task.checkCancellation()
            let worker = Task.detached(priority: .utility) {
                try rateLimits(executable: executable, cancellation: cancellation)
            }
    ...
    static func rateLimits(executable: URL, timeout: TimeInterval = 10,
                           cancellation: ProcessCancellation = ProcessCancellation()) throws -> Data {
        let process = Process()
        ...
        Log.usage.error("codex: app server did not return rate limits")
    ...
    static var handshake: [String] {
        ...
        return [
            #"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"clientInfo":"#
                + #"{"name":"ainotch","title":"AI Notch","version":"\#(version)"}}}"#,
