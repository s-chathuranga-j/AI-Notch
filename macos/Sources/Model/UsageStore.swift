...
final class UsageStore: ObservableObject {
    ...
    @Published var disconnected: Set<String> = [] {
        didSet {
            ...
            // launch, ring and all.
            for id in disconnected.subtracting(oldValue) { invalidate(providerID: id) }
            ...
            for provider in providers where !disconnected.contains(provider.id) {
                if !snapshots.contains(where: { $0.id == provider.id }) { snapshots.append(Self.placeholder(provider)) }
            }
            let order = providers.map(\.id)
            snapshots.sort { (order.firstIndex(of: $0.id) ?? 0) < (order.firstIndex(of: $1.id) ?? 0) }
    ...
    private var providerTasks: [String: Task<ProviderSnapshot, Error>] = [:]

    /// Invalidates even cancellation-unaware results and any held credentials.
    private func invalidate(providerID: String) {
        generations[providerID, default: 0] &+= 1
        providerTasks.removeValue(forKey: providerID)?.cancel()
    ...
    var providerSummaries: [ProviderSummary] {
        providers.map { provider in
            ProviderSummary(id: provider.id, name: provider.displayName,
                            glyph: provider.glyph, account: disconnected.contains(provider.id) ? nil : provider.account(),
    ...
    func stop() {
        ...
        timer = nil
        refreshTask?.cancel()
        for provider in providers { invalidate(providerID: provider.id) }
        isRefreshing = false
    ...
    func refresh() async {
        ...
        for provider in live {
            ...
            guard !disconnected.contains(provider.id) else { continue }
            let generation = generations[provider.id, default: 0]
            if let fresh = await snapshot(from: provider),
               !disconnected.contains(provider.id),
               generations[provider.id, default: 0] == generation, !Task.isCancelled {
    ...
    func refresh(providerID: String) {
        guard let provider = providers.first(where: { $0.id == providerID }),
              !disconnected.contains(providerID), !refreshing.contains(providerID),
              providerTasks[providerID] == nil else { return }
        let generation = generations[providerID, default: 0]
        refreshing.insert(providerID)
        Task { [weak self] in
            guard let self else { return }
            defer {
                if self.providerTasks[providerID] == nil { self.refreshing.remove(providerID) }
            }
            guard self.generations[providerID, default: 0] == generation,
                  let fresh = await self.snapshot(from: provider),
                  !self.disconnected.contains(providerID),
                  self.generations[providerID, default: 0] == generation else { return }
    ...
    func signOut(providerID: String) {
        ...
        disconnected.insert(providerID)
    ...
    func signIn(providerID: String) -> Bool {
        guard !disconnected.contains(providerID),
              let provider = providers.first(where: { $0.id == providerID }) else { return false }
    ...
    ///
    /// AI Notch cannot switch the account itself. The credential belongs to
    ...
    private func snapshot(from provider: UsageProvider) async -> ProviderSnapshot? {
        guard !disconnected.contains(provider.id), !Task.isCancelled,
              providerTasks[provider.id] == nil else { return nil }
        let generation = generations[provider.id, default: 0]
        let task = Task { () throws -> ProviderSnapshot in
            try Task.checkCancellation()
            return try await provider.fetchSnapshot()
        }
        providerTasks[provider.id] = task
        refreshing.insert(provider.id)
        defer {
            if generations[provider.id, default: 0] == generation {
                providerTasks[provider.id] = nil
        ...
        guard !Task.isCancelled, !task.isCancelled,
              !disconnected.contains(provider.id),
              generations[provider.id, default: 0] == generation else {
            if disconnected.contains(provider.id) { provider.forgetCachedCredential() }
            return nil
        }
        switch result {
        ...
        case .failure(let error):
            // Response bodies, tokens and provider error descriptions never enter logs.
            Log.usage.error("provider refresh failed")
