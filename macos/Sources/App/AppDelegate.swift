...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var activityController: ActivityMonitorController?
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Drop response caching from builds that used URLSession.shared.
        URLCache.shared.removeAllCachedResponses()

        let controller = NotchWindowController()

        // `AINOTCH_DEMO=1` puts the design frame's three providers on screen
        // with its numbers, for screenshots and for eyeballing the layout.
        let isDemo = ProcessInfo.processInfo.environment["AINOTCH_DEMO"] == "1"
        do {
            ...
            store.$snapshots
                ...
                .sink { [weak controller] snapshots in
                    withAnimation(NotchMotion.unfold) {
                        controller?.model.snapshots = isDemo ? Fixtures.snapshots() : snapshots
                    }
                    controller?.model.now = Date()
                }
                .store(in: &cancellables)
            if isDemo { controller.model.snapshots = Fixtures.snapshots() }
            else { store.start() }
        ...
        let activityController = ActivityMonitorController(monitors: monitors)
        self.activityController = activityController
        for (id, monitor) in monitors {
            monitor.sessionsPublisher
                .sink { [weak controller, weak activityController] live in
                    let allowed = activityController?.active.contains(id) == true
                    withAnimation(.spring(response: 0.3, dampingFraction: 0.85)) {
                        controller?.model.sessions[id] = allowed ? live : []
                    }
                    controller?.model.now = Date()
                }
                .store(in: &cancellables)
        ...
        preferences?.$disconnectedProviders
            .sink { [weak activityController] in activityController?.apply(disconnected: $0, demo: isDemo) }
            .store(in: &cancellables)
        store?.isBusy = { [weak activityController] in activityController?.isBusy == true }
    ...
    /// The app menu and keyboard shortcut use the same controller as the notch.
