...
- Usage rings and reset windows for Claude Code, Cursor, Codex, Antigravity and GLM.
...
Enabling an account allows local credential/account reads, usage requests and session monitoring for that account. Live usage sends authentication to the relevant provider over HTTPS. Codex launches its installed app server; Antigravity can ask its local language server to refresh remotely. These tools manage their own backend connections.
...
Cursor, Codex, Antigravity and GLM currently expose one account each. Switch accounts in the owning tool to change which one AI Notch reads; simultaneous personal/work rings for these providers are not implemented. GLM uses the first valid credential source in its documented order.
