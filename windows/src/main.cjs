...
app.whenReady().then(async()=>{
 if(demo){accounts=[{id:'claude',kind:'claude',label:'Claude · Personal'},{id:'claude-work',kind:'claude',label:'Claude · Work'},{id:'codex',kind:'codex',label:'Codex'}];config.enabled=accounts.map(a=>a.id);}
 ...
 ipcMain.on('expand',(event,value)=>{if(valid(event)&&typeof value==='boolean'&&!settings){expanded=value;place();}});
 ipcMain.on('settings',event=>{if(valid(event)){settings=!settings;expanded=settings;place();push();}});
