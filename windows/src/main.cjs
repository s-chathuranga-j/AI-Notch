...
function state(){return {demo,settings,expanded,detailId,position:config.position,visibility:config.visibility,accounts:accounts.map(({id,label,kind})=>({id,label,kind,enabled:poller.enabled.has(id),...(rows[id]||{status:'Disabled',windows:[]})}))};}
function push(){if(win&&!win.isDestroyed())win.webContents.send('state',state());}
function place(){
 if(!win)return;
 // Always: the overview stays open. Hidden: the window leaves the screen and
 // the tray is the way back; settings still open on top of either.
 const open=expanded||config.visibility==='always';
 win.setBounds(notchBounds(screen.getPrimaryDisplay().workArea,config.position,accounts.length,open,settings,detailId!==null));
 const hidden=config.visibility==='hidden'&&!settings;
 win.setIgnoreMouseEvents(hidden);
 if(hidden)win.hide();else if(!win.isVisible())win.showInactive();
}
...
app.whenReady().then(async()=>{
 ...
 ipcMain.handle('visibility',async(event,value)=>{if(valid(event)&&VISIBILITIES.includes(value)){config.visibility=value;expanded=value==='always';detailId=null;await save();place();push();}});
 ipcMain.on('expand',(event,value)=>{if(valid(event)&&typeof value==='boolean'&&!settings){expanded=value||config.visibility==='always';if(!expanded)detailId=null;place();}});
 ...
 await win.loadFile(path.join(__dirname,'index.html'));place();push();await refresh();
